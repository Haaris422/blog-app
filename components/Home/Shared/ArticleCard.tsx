...
export function ArticleCard({ className, article, isFeatured }: ArticleCardProps) {
  return (
    <div
      key={article.id}
      className={`flex w-full bg-white/90 transition-all duration-300 ease-in-out hover:shadow-[0_0_10px_rgba(100,100,100,0.4)] hover:scale-105`}
    >
      ...
      <div className="w-full h-full text-black p-2">
        <div className="flex justify-between">
          <Link href={'#'} className={`${animationCalss} hover:underline`}>
            <h2 className="text-2xl font-bold">{article.title}</h2>
          </Link>
        </div>

        <h3 className="text-sm italic">{article.sub_title}</h3>
        <p className="text-justify h-[70%] pt-4 line-clamp-3">{article.content}</p>
