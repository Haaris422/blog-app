...
export function ArticleCard({ className, article, isFeatured }: ArticleCardProps) {
  return (
    <div
      key={article.id}
      className={`flex flex-col sm:flex-row max-w-[300px] sm:max-w-[1000px] sm:max-h-[250px] shadow-[0px_0px_10px_rgba(255,255,255,0.9)] bg-white/90 transition-all duration-300 ease-in-out hover:scale-105`}
    >
      <div className="relative max-w-[300px] sm:max-w-44 md:max-w-52">
        <img
          src={article.image}
      ...
      <div className="w-full flex sm:text-left text-center justify-between flex-col max-h-[100%] text-black p-2">
        <div className="">
          <Link href={`/articles/${article.slug}`} className={`${animationCalss} text-2xl font-bold hover:underline`}>
            {article.title}
          </Link>
